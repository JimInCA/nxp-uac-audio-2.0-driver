...
static const io_port_t gpioLEDBits[NUM_LEDS] = {{3, 5}, {0, 7}, {3, 7}, {0, 8}};
...
static void Board_LED_Init()
{
	...
	for (idx = 0; idx < (sizeof(gpioLEDBits) / sizeof(io_port_t)); ++idx) {
		...
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, gpioLEDBits[idx].port, gpioLEDBits[idx].pin, (bool) (idx < 3 ? true : false));
	}
}
...
void Board_LED_Set(uint8_t LEDNumber, bool On)
{
	if (LEDNumber < (sizeof(gpioLEDBits) / sizeof(io_port_t)))
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, gpioLEDBits[LEDNumber].port, gpioLEDBits[LEDNumber].pin, (bool) (LEDNumber == 3 ? On : !On));
}
...
static Status Board_Audio_CodecInit(void)
{
	//printf("%s()\r\n", __FUNCTION__);
...
void Board_Audio_Init(LPC_I2S_T *pI2S)
{
	...
	if (pI2S == LPC_I2S0) {
		Chip_SCU_PinMuxSet (0x3, 0,  (SCU_PINIO_FAST | SCU_MODE_FUNC2)); /* I2S0_TX_SCK */
		Chip_SCU_PinMuxSet (0x3, 1,  (SCU_PINIO_FAST | SCU_MODE_FUNC0)); /* I2S0_TX_WS  */
		Chip_SCU_PinMuxSet (0x3, 2,  (SCU_PINIO_FAST | SCU_MODE_FUNC0)); /* I2S0_TX_SDA */
        /* I'm not sure if the following clocks need to be setup. */
	...
	/* Init WM8904 CODEC */
	while (Board_Audio_CodecInit() != SUCCESS) {}
}
