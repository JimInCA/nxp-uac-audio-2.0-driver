	...
	/* Enums: */
		...
		enum Audio_Descriptor_ClassSubclassProtocol_t
		{
			...
			AUDIO_CSCP_ControlProtocol                = 0x00, /**< Descriptor Protocol value indicating that the device or
			                                                   ...
			                                                   */
#else
			AUDIO_CSCP_ControlProtocol                = 0x20, /**< Descriptor Protocol value indicating that the device or
			                                                   *   interface belongs to the Audio Control protocol.
			                                                   */
#endif
			...
			AUDIO_CSCP_StreamingProtocol              = 0x00, /**< Descriptor Protocol value indicating that the device or
			                                                   ...
			                                                   */
#else
			AUDIO_CSCP_StreamingProtocol              = 0x20, /**< Descriptor Protocol value indicating that the device or
			                                                   *   interface belongs to the Streaming Audio protocol.
			                                                   */
#endif
		};
...
#else
			AUDIO_DSUBTYPE_CSInterface_Effect               = 0x07, /**< Audio class specific control interface Effect Unit. */
			AUDIO_DSUBTYPE_CSInterface_Processing           = 0x08, /**< Audio class specific control interface Processing Unit. */
			AUDIO_DSUBTYPE_CSInterface_Extension            = 0x09, /**< Audio class specific control interface Extension Unit. */
			AUDIO_DSUBTYPE_CSInterface_ClockSource          = 0x0A, /**< Audio class specific control interface Clock Source. */
			AUDIO_DSUBTYPE_CSInterface_ClockSelector        = 0x0B, /**< Audio class specific control interface Clock Selector. */
			AUDIO_DSUBTYPE_CSInterface_ClockMultiplier      = 0x0C, /**< Audio class specific control interface Clock Multiplier. */
			AUDIO_DSUBTYPE_CSInterface_SampleRateConverter  = 0x0D, /**< Audio class specific control interface Sample RateConverter. */
#endif
		};
...
#else
			AUDIO_DSUBTYPE_CSInterface_Encoder        = 0x03, /**< Audio class specific streaming interface encoder descriptor. */
			AUDIO_DSUBTYPE_CSInterface_Decoder        = 0x04, /**< Audio class specific streaming interface decoder descriptor. */
#endif
		};
...
#else
			AUDIO_REQ_Cur           = 0x01, /**< Audio class-specific request to manipulate the current setting of a Control. */
			AUDIO_REQ_Range         = 0x02, /**< Audio class-specific request to provide information about the limitations of Control. */
			AUDIO_REQ_Mem           = 0x03, /**< Audio class-specific request to interact with an addressable Entity. */
#endif
		};

#ifdef USB_AUDIO_2DOT0
		/** Enum for Audio class specific clock source control selectors which can be set and retrieved by a USB host. */
        /*  ?.? / A.17.1 */
		enum Audio_ClockControlSelectors_t
		{
			AUDIO_CS_SAM_FREQ_CONTROL = 0x01, /**< Audio class-specific request to get or set Sample Frequency. */
			AUDIO_CS_CLOCK_VALID      = 0x02, /**< Audio class-specific request to indicate clock source is valid. */
		};
#endif
...
#else
			AUDIO_EPCONTROL_Pitch        = 0x01, /**< Pitch adjustment of the endpoint. */
			AUDIO_EPCONTROL_OverRun      = 0x02, /**< ???. */
			AUDIO_EPCONTROL_UnderRun     = 0x03, /**< ???. */
#endif
		};
	...
	/* Type Defines: */
		...
		typedef ATTR_IAR_PACKED struct
		{
			...
			uint8_t                 AssociatedOutputTerminal; /**< ID of associated output terminal, for physically grouped terminals
			                                                   ...
			                                                   */
#ifdef USB_AUDIO_2DOT0
            uint8_t                 bCSourceID; /* ID of Clock Entity to which this Input Terminal is connected. */
#endif
...
#ifdef USB_AUDIO_2DOT0
            uint16_t                bmControls; /* Bitmap controls */
#endif
		...
		typedef ATTR_IAR_PACKED struct
		{
			...
			uint8_t  bAssocTerminal; /**< ID of associated output terminal, for physically grouped terminals
			                          ...
			                          */
#ifdef USB_AUDIO_2DOT0
            uint8_t  bCSourceID; /* ID of Clock Enity to which this Input Terminal is connected. */
#endif
...
#else
			uint32_t wChannelConfig; /**< \c CHANNEL_* masks indicating what channel layout is supported by this terminal. */
#endif
			uint8_t  iChannelNames; /**< Index of a string descriptor describing this channel within the device. */
#ifdef USB_AUDIO_2DOT0
            uint16_t bmControls; /* Bitmapped controls */
#endif
...
#ifdef USB_AUDIO_2DOT0
            uint8_t  bCSourceID; /* ID of the Clock Enity to which this output Terminal is connected. */
            uint16_t bmControls; /* Bitmapped Controles */
#endif
			uint8_t                 TerminalStrIndex; /**< Index of a string descriptor describing this descriptor within the device. */
...
#else       
            uint8_t  bCategory; /** Indicating the primary use of this audio function, as intended by the manufacturer. */
		...
		typedef ATTR_IAR_PACKED struct
		{
			...
			uint8_t  bDescriptorSubtype;/**< Sub type value used to distinguish between audio class-specific descriptors,
			                             *   a value from the @ref Audio_CSInterface_AS_SubTypes_t enum.
			                             */
			uint8_t  bClockID; /* Constant uniquely identifying the Clock Source Entity within the audio function.
			                    * This value is used in all requests to address this Entity.
			                    */

			uint8_t  bmAttributes; /* Bitmap: D1..0: Clock Type: 00: External Clock
		...
		} ATTR_PACKED USB_Audio_StdDescriptor_Source_Clock_t;
#endif
...
#else
			uint8_t                 ChannelControls[4]; /**< Feature masks for the control channel, and each separate audio channel. */
#endif
...
#else
			uint8_t bmaControls[8]; /**< Feature masks for the control channel, and each separate audio channel. */
#endif
		...
		typedef ATTR_IAR_PACKED struct
		{
            ...
            uint32_t                bmFormats; /** The Audio Data Format(s) that can be used to communicate with this interface.
		 ...
		 *  @note Regardless of CPU architecture, these values should be stored as little endian.
		 */
        /* Format Document: Table 2-1 / Table 2-2 */
		typedef ATTR_IAR_PACKED struct
		{
			USB_Descriptor_Header_t Header; /**< Regular descriptor header containing the descriptor's type and length. */
			uint8_t                 Subtype; /**< Sub type value used to distinguish between audio class-specific descriptors,
		 ...
		 *  @note Regardless of CPU architecture, these values should be stored as little endian.
		 */
        /* Format Document: Table 2-3 / Removed in ADC V2.0 */
		typedef ATTR_IAR_PACKED struct
		{
